...
1. Phase 1 learning rate and epochs. Find empirically: sweep LR in {1e-5, 3e-5, 1e-4}, train until success criteria, pick the smallest LR that meets them.
...
4. Phase 4 relearning protocol: learning rate, template count (proposed 5), evaluation interval (proposed 5 steps), step cap, recovery threshold (proposed: forward accuracy reaches 90% of M1's). Pre-register before running.
