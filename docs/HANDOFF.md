...
**Phase 1, bidirectional training.** Full finetune M0 → M1 on all 150 entities, both directions of D, forward of S, with general-text replay mixed in (decision 18). Success criteria: held-out accuracy ≥ 90% in both directions on both the forget and retain sets, margin well above zero, **and** WikiText perplexity ≤ 73.1 (1.5x base). This is the critical departure from Berglund, who trained one direction. The experiment requires verified bidirectional knowledge. Save M1 as a Kaggle Dataset so every later phase starts from it without retraining. This is the first real milestone.
...
- **Utility collapse.** Aggressive unlearning wrecks the model. C2. Do not skip. Observed 2026-09-18 in *Phase 1*, not Phase 2, which the original plan did not anticipate: plain finetuning on the synthetic set destroyed the base model before any unlearning happened. Fixed by replay (decision 18). The general lesson is that C2 needs to be checked at every phase that touches weights, not only after unlearning.
